...
void optimizeSurfaceNet_1_cuda(GpuBuffer<float> &verts_, const GpuBuffer<int32_t> &vert2tris_, const GpuBuffer<int32_t> &tris_) {
  ...
  */
  ...
  int V = verts_.allocatedCnt / 3;
  //float alpha = .6; int iters = 5;
  //float alpha = .9; int iters = 20;
  //float alpha = .95; int iters = 50;
  float alpha = .98; int iters = 200;
  //float alpha = .65; int iters = 1;


  GpuBuffer<float> verts_2;
  verts_2.allocate(verts_.allocatedCnt);
  cudaMemcpy(verts_2.buf, verts_.buf, sizeof(float)*verts_.allocatedCnt, cudaMemcpyDeviceToDevice);

  float* verts1 = verts_.buf;
  float* verts2 = verts_2.buf;
  const int32_t* vert2tris = vert2tris_.buf;
  const int32_t* tris = (const int32_t*) tris_.buf;

  std::cout << " - [SurfaceNet] running " << iters << " iterations...\n";
  ...
  std::cout << " - [SurfaceNet] ... done\n";

  if (iters % 2 == 0) {
    cudaMemcpy(verts_.buf, verts_2.buf, sizeof(float)*verts_.allocatedCnt, cudaMemcpyDeviceToDevice);
    //verts_.copy_(verts_2);
  }
